...
def _checkin(today: str):
    """
    Full check-in flow for a single day.
    Uses `return` to exit early — never calls sys.exit() inside the with block,
    so the context manager always closes the transaction cleanly.
    """
    ...
    # Resumen fuera del with: la transacción ya se commiteó
